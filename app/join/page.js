...
// 🔥 1. We move the logic into a "Content" component
      ...
      <div className="max-w-md w-full bg-white rounded-[2.5rem] p-10 border border-rose-50 shadow-2xl">
        <form onSubmit={handleJoin} className="space-y-8">
          ...
          <button 
            ...
            className="w-full py-5 rounded-2xl bg-slate-900 text-white font-black text-xl hover:bg-rose-500 transition-all disabled:opacity-50 cursor-pointer"
          >
            {loading ? "Verifying..." : "Enter Space ✨"}
          </button>

          {error && (
            <p className="text-rose-500 text-[10px] font-black text-center uppercase tracking-widest bg-rose-50 p-3 rounded-xl border border-rose-100">
                {error}
            </p>
          )}
          
          <Link href="/roles" className="block text-center text-slate-400 font-bold text-[10px] uppercase tracking-widest hover:text-slate-600 transition-colors">
            Back to Roles
          </Link>
        </form>
      </div>
...
// 🔥 2. This is the main export that Vercel looks for
export default function JoinSpace() {
  return (
    <Suspense fallback={
        <div className="min-h-screen bg-slate-900 flex items-center justify-center">
            <p className="text-rose-400 font-black animate-pulse">CONNECTING...</p>
        </div>
    }>
      <JoinContent />
    </Suspense>
