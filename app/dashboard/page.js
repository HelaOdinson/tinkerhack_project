...
export default function Dashboard() {
  ...
  const nextPic = (e) => { e.stopPropagation(); setCurrentPic(prev => (prev % 3) + 1); };
  const prevPic = (e) => { e.stopPropagation(); setCurrentPic(prev => (prev === 1 ? 3 : prev - 1)); };
        ...
        <nav className="flex-1 space-y-2">
          {['Dashboard', 'Memories', 'Settings'].map((item) => (
            <button key={item} className={`flex items-center gap-4 w-full px-3 py-3 rounded-xl hover:bg-white/60 transition-all font-bold text-sm ${currentTheme.text} ${isSidebarExpanded ? 'justify-start px-4' : 'justify-center'}`}>
              <span className="text-xl flex-shrink-0 w-6 text-center">{item === 'Dashboard' ? '🏡' : item === 'Memories' ? '✨' : '⚙️'}</span>
          ...
          <aside className="md:col-span-3 space-y-6">
            <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl text-center">
               <div className="space-y-4">
                  <div><p className="text-[10px] font-black opacity-40 uppercase">Local</p><p className="text-2xl font-black">{timeHome}</p></div>
                  <div className="h-[1px] bg-white/10 w-full"></div>
                  <div><p className="text-[10px] font-black text-amber-300 uppercase">Her</p><p className="text-2xl font-black">{timeAway}</p></div>
               </div>
            </div>
